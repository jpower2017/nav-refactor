...
class Nav extends Component {
  ...
  fStyleNew = (show = 0, id = 0) => {
    console.log("fstylenew f " + show);
    //return "slide-out";
    return show ? "test  xx slide-out" : null;
  };
  ...
  getFirstChild = item => {
    return R.prop("id", R.find(x => x.parentId == item.id, this.state.data));
  };
  ...
  onclick = item => {
    ...
    this.setState({
      currentSelection: item.leaf ? item.id : this.getFirstChild(item)
    });
    ...
    if (this.hasChildrenShowing(item)) {
      this.setState({
        data: R.compose(
          cUpdateChildren(false),
          cUpdateSibs(true),
          cSetChildrenShowing(false)
        )(this.state.data)
      });
      ...
      this.setState({
        data: R.compose(
          cUpdateChildren(true),
          cUpdateSibs(false),
          cSetChildrenShowing(true)
        )(this.state.data)
      });
    }
  };
      ...
      <div style={{ padding: "0px" }}>
        {this.state.data.map((x, i) => (
          <Item
            ...
            styl={this.fStyleNew(x.childrenShowing)}
            item={x}
            onclick={this.onclick}
            currentSelection={
              this.state.currentSelection == x.id ? true : false
            }
            currentParent={this.state.level1Selection == x.id ? true : false}
            childrenShowing={x.childrenShowing}
            leaf={x.leaf}
            show={x.show}
          />
        ))}
